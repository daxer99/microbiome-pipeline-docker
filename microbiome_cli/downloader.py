...
def download_kneaddata(db_dir):
    ...
    # Usa micromamba run o directamente kneaddata_database (ya está en PATH)
    cmd = f"micromamba run -n microbiome-pipeline kneaddata_database --download human_genome bowtie2 {db_dir}"
    run_cmd(cmd)
...
def download_metaphlan(db_dir):
    db_dir = str(Path(db_dir).resolve())
    cmd = f"micromamba run -n microbiome-pipeline metaphlan --install --index mpa_vJun23_CHOCOPhlAnSGB_202307 --db_dir {db_dir}"
    run_cmd(cmd)
...
def download_chocophlan(db_dir):
    db_dir = str(Path(db_dir).resolve())
    cmd = f"micromamba run -n microbiome-pipeline humann_databases --download chocophlan full {db_dir}"
    run_cmd(cmd)
...
def download_uniref(db_dir):
    db_dir = str(Path(db_dir).resolve())
    cmd = f"micromamba run -n microbiome-pipeline humann_databases --download uniref uniref90_diamond {db_dir}"
    run_cmd(cmd)
...
def download_utility_mapping(db_dir):
    db_dir = str(Path(db_dir).resolve())
    cmd = f"micromamba run -n microbiome-pipeline humann_databases --download utility_mapping full {db_dir}"
    run_cmd(cmd)
